...
$(TARGET): $(OBJECTS) | $(BINDIR)
	...
	@if [ ! -f $(BINDIR)/settings.ini ]; then \
		echo "# Mario Game Settings" > $(BINDIR)/settings.ini; \
		echo "[Video]" >> $(BINDIR)/settings.ini; \
		echo "width=800" >> $(BINDIR)/settings.ini; \
		echo "height=600" >> $(BINDIR)/settings.ini; \
		echo "fullscreen=false" >> $(BINDIR)/settings.ini; \
		echo "" >> $(BINDIR)/settings.ini; \
		echo "[Audio]" >> $(BINDIR)/settings.ini; \
		echo "music_volume=100" >> $(BINDIR)/settings.ini; \
		echo "sound_volume=100" >> $(BINDIR)/settings.ini; \
		echo "" >> $(BINDIR)/settings.ini; \
		echo "[Controls]" >> $(BINDIR)/settings.ini; \
		echo "jump=32" >> $(BINDIR)/settings.ini; \
		echo "run=304" >> $(BINDIR)/settings.ini; \
		echo "left=276" >> $(BINDIR)/settings.ini; \
		echo "right=275" >> $(BINDIR)/settings.ini; \
		echo "down=274" >> $(BINDIR)/settings.ini; \
...
generate-settings:
	...
	@mkdir -p $(BINDIR)
	@echo "# Mario Game Settings" > $(BINDIR)/settings.ini
	@echo "# This file is automatically generated" >> $(BINDIR)/settings.ini
	@echo "" >> $(BINDIR)/settings.ini
	@echo "[Video]" >> $(BINDIR)/settings.ini
	@echo "width=800" >> $(BINDIR)/settings.ini
	@echo "height=600" >> $(BINDIR)/settings.ini
	@echo "fullscreen=false" >> $(BINDIR)/settings.ini
	@echo "vsync=true" >> $(BINDIR)/settings.ini
	@echo "" >> $(BINDIR)/settings.ini
	@echo "[Audio]" >> $(BINDIR)/settings.ini
	@echo "music_volume=100" >> $(BINDIR)/settings.ini
	@echo "sound_volume=100" >> $(BINDIR)/settings.ini
	@echo "mute=false" >> $(BINDIR)/settings.ini
	@echo "" >> $(BINDIR)/settings.ini
	@echo "[Controls]" >> $(BINDIR)/settings.ini
	@echo "# SDL Keycodes: Space=32, Shift=304, Left=276, Right=275, Down=274, Up=273" >> $(BINDIR)/settings.ini
	@echo "jump=32" >> $(BINDIR)/settings.ini
	@echo "run=304" >> $(BINDIR)/settings.ini
	@echo "left=276" >> $(BINDIR)/settings.ini
	@echo "right=275" >> $(BINDIR)/settings.ini
	@echo "down=274" >> $(BINDIR)/settings.ini
	@echo "up=273" >> $(BINDIR)/settings.ini
	@echo "" >> $(BINDIR)/settings.ini
	@echo "[Game]" >> $(BINDIR)/settings.ini
	@echo "debug=false" >> $(BINDIR)/settings.ini
	@echo "starting_lives=3" >> $(BINDIR)/settings.ini
